...
namespace cs
{
    public class MyException : ApplicationException
    {
        ...
        public MyException() : base("ошибка ввода")
        {
            TimeException = DateTime.Now;
        }
        internal class Exception
        {
            static void throw_funct()
            {
            ...
            static void trying_funct()
            {
                try
                {
                    funct();
                }
                catch (System.Exception exclusion)
                {
                    WriteLine(exclusion.Message);
                    WriteLine($"Stack trace: {exclusion.StackTrace}");
                }
            }
        }
    }
    ...
    class Wash_powd : Hous_chem
    {
        ...
        public void data_output()
        {
            try
            {
                ...
                if (hous_chem_2.entr_per > hous_chem_2.per_of_write_downs || hous_chem_2.sale_per > hous_chem_2.per_of_write_downs || hous_chem_2.entr_per > hous_chem_2.sale_per) throw new MyException();
            }
            catch (DivideByZeroException exception)
            {
                WriteLine("ошибка информации о товаре");
                if (hous_chem_2.per_of_write_downs == 0) WriteLine(exception.Message);
            }
        }
    }
    class Menu
